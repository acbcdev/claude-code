...
[ -n "$EFFORT" ] && MODEL_FMT="$MODEL ($EFFORT)" || MODEL_FMT="$MODEL"
...
echo "$FOLDER/ | $MODEL_FMT | ${USED_K}k/${TOTAL_K}k (${PERCENT}%) | $COST_FMT - ${API_TIME_SEC}.${API_TIME_DEC}s | +${LINES_ADDED} -${LINES_REMOVED}$GIT_INFO"
